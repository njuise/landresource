...
class LandCrawler:
    ...
    def get_next_page(self):
        ...
        try:
            ...
            nextPageElem = WebDriverWait(self.driver, 20).until(EC.element_to_be_clickable((By.LINK_TEXT, u'下页')))
    ...
    def get_appropriate_page(self, start_date, start_page):
        ...
        while True:
            ...
            total_page = 0
            try:
                #driver.find_element_by_xpath("//td[@class='pager']")
                #driver.find_element_by_xpath("//table[@class='ContentItem']").find_element_by_xpath("//table")
                #driver.find_element_by_xpath(u"//table[@class='ContentItem']//table[@class='grid']//td[@class='pager'][starts-with(text(),'共')]")
                page_elem = driver.find_element_by_xpath(
                    u"//table[@class='ContentItem']//table[@class='grid']//td[@class='pager'][starts-with(text(),'共')]")
            ...
            print(self.tool.getCurTime(), end_date,
                  u"为终止日期，共有", total_page, u"页数据\r\n")
        ...
        if page_elem is not None and page_elem != "":
            try:
                page_input = page_elem.find_element_by_xpath(
                    "..//input[last()-1]")
    ...
    def get_new_main_page(self, cur_ed):
        self.get_main_page()
        self.cur_start_date = self.time_ranger.get_next_start_date(cur_ed)
    ...
    def parse_main_table(self):
        ...
        try:
            ...
            elems_with_url = table.find_elements_by_xpath("//a[starts-with(@href, 'default.aspx')]")
        ...
        for elem in elems_with_url:
            ...
            if district.find('...') >= 0:
                try:
                    district = elem.find_element_by_xpath(
                        "../../td[2]/span").get_attribute('title')
                except NoSuchElementException:
                    district = elem.find_element_by_xpath("../../td[2]").text
    ...
    def save_detail_pages(self, length):
        ...
        for index in range(length):
            ...
            print(self.tool.getCurTime(), u"处理进度：",
                  index+1, u"/", length, "\r\n")
    ...
    def parse_detail_page(self):
      ...
      #   print district
        ...
        detail = {
            ...
            'area': "0.0" if area is None or area == "" else area, 'date': date, 'price': "0.0" if price is None or price == "" else price, 'purpose': purpose, 'supply': supply, 'location': location
        }
        return detail
...
if __name__ == "__main__":
    ...
    #start_p = input(u"请输入开始页码（若不确定则输入1）,以回车结束：".encode("gbk"))
        ...
        d.saveTable()
        redirect.restore()
        print(u"数据获取完毕,结果以excel格式输出，过程日志可参考err",
              starttime, u".txt和out", starttime, u".txt文件")
        input(u"输入回车以结束".encode("gbk"))
